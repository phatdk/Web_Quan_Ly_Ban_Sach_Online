		...
		public async Task<List<PointTranHistoryViewModel>> GetByUser(int userId)
		{
			...
			var objlist = (from a in histories
						   ...
						   select new PointTranHistoryViewModel()
						   {
							   ...
							   Code = b1 != null ? b1.Code : c1 != null ? c1.Code : "",
							   Notif = a.Point_Amount_Userd > 0 ?
							   "Nhận được " + a.Point_Amount_Userd + " điểm từ giao dịch mua hàng " + b1.Code : 
							   a.Id_Order != null ?
							   "Sử dụng " + Math.Abs(a.Point_Amount_Userd) + " điểm cho đơn hàng " + b1.Code : 
							   "Sử dụng " + Math.Abs(a.Point_Amount_Userd) + " điểm cho cho giao dịch đổi mã khuyến mãi " + c1.Name,
						   }).ToList();
			return objlist;
		}
	}
