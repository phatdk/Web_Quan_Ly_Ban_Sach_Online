	...
	public class BookService : IBookService
	{
		...
		public async Task<bool> Update(UpdateBookModel requet)
		{
			try
			{
				...
				foreach (var item in requet.authorSelected) // tạo phần tử mới
				{
				...
				foreach (var item in bookgenres)// Loại bỏ phần tử không có và skip các phần tử đã có
				{
				...
				foreach (var item in requet.genreSelected) // tạo phần tử mới
				{
		...
		public async Task<BookViewModel> GetById(int id)
		{
			...
			return new BookViewModel()
			{
				...
				SupplierName = ((await _supplierRepository.GetAllAsync()).Where(x=>x.Id == books.Id_Supplier).FirstOrDefault().Name) ?? "",
