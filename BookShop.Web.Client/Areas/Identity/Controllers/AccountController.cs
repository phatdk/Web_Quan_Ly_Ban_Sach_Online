		...
		public async Task<IActionResult> Login(LoginViewModel model)
		{
			...
			if (ModelState.IsValid)
			{
				...
				if (result.Succeeded)
				{
					...
					{
						var customCart = JsonConvert.DeserializeObject<List<CartDetailViewModel>>(cartSession);
						foreach (var item in customCart)
						{
							var pc = (await _CartDetailService.GetByCart(userLog.Id)).FirstOrDefault(p => p.Id_Product == item.Id_Product);
							if (pc == null)
							{
